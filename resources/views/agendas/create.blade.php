<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Buat Agenda Baru</h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex items-center gap-3 mb-5 px-1">
                <div class="flex items-center gap-2">
                    <span
                        class="h-6 w-6 rounded-full bg-blue-600 text-white text-xs font-bold flex items-center justify-center">1</span>
                    <span class="text-sm font-semibold text-gray-700">Peserta &amp; Detail Agenda</span>
                </div>
                <div class="flex-1 h-px bg-gray-200"></div>
                <div class="flex items-center gap-2">
                    <span
                        class="h-6 w-6 rounded-full bg-gray-100 text-gray-400 text-xs font-bold flex items-center justify-center">2</span>
                    <span class="text-sm font-medium text-gray-400">Rincian Biaya</span>
                </div>
            </div>
            ...
            <form action="{{ route('agendas.store') }}" method="POST" id="agenda-form">
                @csrf

                @php
                    $oldPesertaIds = old('pegawai_id', []);

                    // Replika logika pengurutan dari PegawaiController@index, supaya urutan
                    // peserta di sini persis sama dengan urutan di halaman Data Pegawai:
                    // dikelompokkan per unit kerja (alfabet, unit kosong ditaruh paling bawah),
                    // lalu di dalam tiap unit: PNS diurutkan urutan→golongan_rank,
                    // Non PNS diurutkan urutan→nama.
                ...
                <div class="grid grid-cols-1 lg:grid-cols-5 gap-6 lg:items-stretch">
                    ...
                    {{-- KOLOM KIRI: pilih peserta (langkah pertama; sticky, tinggi menyesuaikan kolom kanan) --}}
                    ...
                    <div
                        ...
                        <div>
                            ...
                            <input type="text" name="nomor_st" value="{{ old('nomor_st') }}" required
                                placeholder="800/123/ST/2026"
                        ...
                        <div id="nomor-st-karo-wrap" class="hidden rounded-lg transition-shadow duration-500">
                            ...
                            <input type="text" id="nomor-st-karo-input" name="nomor_st_karo"
                                value="{{ old('nomor_st_karo') }}" placeholder="800/124/ST/2026"
                                class="w-full border border-gray-200 rounded-lg px-3.5 py-2.5 text-sm focus:border-blue-400 focus:ring-1 focus:ring-blue-400 outline-none transition placeholder:text-gray-300">
                            ...
                            <div>
                                ...
                                <select name="tujuan" id="tujuan-select" required data-placeholder="Cari provinsi..."
                                    ...
                                    @foreach ($provinsiList as $provinsi)
                                        <option value="{{ $provinsi }}" @selected(old('tujuan') === $provinsi)>{{ $provinsi }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="sm:w-1/2 sm:pr-2">
                            ...
                            <div class="relative">
                                <select name="alat_angkut" required
                                    ...
                                    <option value="" disabled {{ old('alat_angkut') ? '' : 'selected' }}>— Pilih —
                                    </option>
                                    <option value="darat" @selected(old('alat_angkut') === 'darat')>Angkutan Darat</option>
                                    <option value="udara" @selected(old('alat_angkut') === 'udara')>Angkutan Udara</option>
                                    <option value="laut" @selected(old('alat_angkut') === 'laut')>Angkutan Laut</option>
                                    <option value="darat_udara" @selected(old('alat_angkut') === 'darat_udara')>Angkutan Darat dan Udara</option>
                                </select>
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    ...
                                    <div class="relative">
                                        <select name="ppk_id" required data-placeholder="Cari PPK..."
                                            ...
                                            @forelse ($ppkList as $p)
                                                <option value="{{ $p->id }}" {{ old('ppk_id') == $p->id ? 'selected' : '' }}>
                                                    {{ $p->nama_gelar ?? $p->nama }}
                                                </option>
                                            @empty
                                    ...
                                    <div class="relative">
                                        <select name="bendahara_id" required data-placeholder="Cari Bendahara..."
                                            ...
                                            @forelse ($bendaharaList as $p)
                                                <option value="{{ $p->id }}" {{ old('bendahara_id') == $p->id ? 'selected' : '' }}>
                                                    {{ $p->nama_gelar ?? $p->nama }}
                                                </option>
                                            @empty
                        ...
                        <div class="mt-8 pt-6 border-t border-gray-100">
                            ...
                            <div id="anggaran-body" class="overflow-hidden transition-all duration-300 ease-out">
                                <div class="pt-5 space-y-5">
                                    @include('agendas.partials.administrasi-anggaran', ['agenda' => null])
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex items-center justify-between mt-6">
                    <a href="{{ route('agendas.index') }}" class="text-sm text-gray-400 hover:text-gray-600 transition">
                        &larr; Batal
                    </a>
                    <button type="submit"
    ...
    <script>
        ...
        // Return true kalau memang baru muncul.
        ...
        document.querySelectorAll('.peserta-checkbox').forEach(function (cb) {
            cb.addEventListener('change', updatePesertaCount);
        });
        ...
        (function () {
            ...
            const oldKota = @json(old('kota_tujuan'));
            ...
            function renderKotaOptions() {
                ...
                list.forEach(function (nama) {
                    ...
                    if (oldKota && oldKota === nama) opt.selected = true;
